...
def ensure_lambda_pull_policy(ecr_client, repository_name):
    """
    Unconditionally add a repository policy statement allowing Lambda to pull images.
    """
    try:
        resp = ecr_client.get_repository_policy(repositoryName=repository_name)
        policy = json.loads(resp['policyText'])
    except ecr_client.exceptions.RepositoryPolicyNotFoundException:
        # No policy yet: start fresh
    ...
    # Always append (you may end up with duplicates)
    ...
    print(f"✅ (Re‑)Added Lambda pull policy to ECR repo '{repository_name}'")
...
def deploy_lambda_container(ecr_image_uri, function_name, role_arn, region="us-east-1", memory_size=1024, timeout=90, api_gateway=False, api_name=None, stage_name="prod"):
    ...
    repo = ecr_image_uri.split("/")[-1].split(":")[0]
    print(f"⏳ Ensuring Lambda pull policy on ECR repo '{repo}'…")
    ensure_lambda_pull_policy(ecr, repo)
...
def main():
    ...
    parser.add_argument('--function-name', required=False, help='Name for the Lambda function')
    parser.add_argument('--role-arn', required=False, help='ARN of the Lambda execution role')
    ...
    parser.add_argument('--build-only', action='store_true', help='Only build and push the Docker image, do not deploy Lambda')
    parser.add_argument('--build-image', action='store_true', help='Build the Docker image before deployment, even if image-uri is provided')
    
    args = parser.parse_args()
    
    # First, check if we're in build-only mode
    if args.build_only:
        ...
        # — now enforce the Lambda‑pull policy on the repo —
        ecr = boto3.client("ecr", region_name=args.region)
        repo = ecr_image_uri.split("/")[-1].split(":")[0]
        print("Ensuring Lambda permissions on the ECR repo (build‑only)…")
        ensure_lambda_pull_policy(ecr, repo)

        # Optional: dump out the current policy so you can see it right away
        policy_txt = ecr.get_repository_policy(repositoryName=repo)["policyText"]
        print("🔍 Current repo policy:", policy_txt)

        sys.exit(0)

    # For deployment, function name and role ARN are required
    if not args.function_name:
        parser.error("--function-name is required for deployment")
    if not args.role_arn:
        parser.error("--role-arn is required for deployment")
    
    # Determine if we need to build and push a container or use the provided URI
    if args.build_image or not args.image_uri:
        ecr_image_uri = build_and_push_container()
        if not ecr_image_uri:
            print("Failed to build and push container. Exiting.")
            sys.exit(1)
